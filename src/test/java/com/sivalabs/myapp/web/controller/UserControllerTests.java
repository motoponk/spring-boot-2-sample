...
public class UserControllerTests {
    ...
    public void should_get_user_by_id() throws Exception {
        GitHubUserDTO gitHubUserDTO = GitHubUserDTO.builder()
                        ...
                        .repos(asList(GitHubRepoDTO.builder().build()))
                        .build();

        UserProfile profile = new UserProfile();
        profile.setGithubProfile(gitHubUserDTO);
        profile.setId(existingUser.getId());
        profile.setName(existingUser.getName());
        profile.setEmail(existingUser.getEmail());

        given(userService.getUserProfile(existingUser.getId())).willReturn(Optional.of(profile));
    ...
    public void should_delete_user() throws Exception {
        willDoNothing().given(userService).deleteUser(existingUser.getId());

        this.mockMvc.perform(
                delete("/api/users/" + existingUser.getId())
                )
                .andExpect(status().isOk());
    }
}
